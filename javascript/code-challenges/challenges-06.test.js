...
const addValues = (arr) => arr.reduce((sum, current) => sum + current, 0);
...
const countNumberOfElements = (arr) => arr.reduce(count => count + 1, 0);
...
const countNumberOfChildren = (arr) => arr.reduce((count, character) => count + (character.children ? character.children.length : 0), 0);
...
const calculateAverage = (arr) => {
  const { sum, count } = arr.reduce((acc, value) => {
    acc.sum += value;
    acc.count += 1;
    return acc;
  }, { sum: 0, count: 0 });
...
const extractStats = (snorlaxData) => {
  return snorlaxData.stats.reduce((acc, { stat, baseStat }) => {
    acc[stat.name] = baseStat;
    return acc;
  }, {});
};
...
const extractChildren = (arr) => {
  return arr.filter(character => character.name.includes('a'))
            .reduce((childrenNames, character) => character.children ? [...childrenNames, ...character.children] : childrenNames, []);
};
