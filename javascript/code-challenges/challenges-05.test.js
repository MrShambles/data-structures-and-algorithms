...
const hasChildrenValues = (arr, character) => {
  const characterObj = arr.find(item => item.name === character);
...
const hasChildrenEntries = (arr, character) => {
  const characterObj = arr.find(item => item.name === character);
  if (!characterObj) return false; // Return false if the character isn't found

  // Use Object.entries() to check if there's an entry for 'children' with length > 0
  return Object.entries(characterObj).some(([key, value]) => key === 'children' && Array.isArray(value) && value.length > 0);
};
...
const totalCharacters = (arr) => {
  return arr.reduce((total, character) => {
    // Start with 1 for the character themselves
    let count = 1;
    // Add one if the character has a spouse
    if (character.spouse) count += 1;
    // Add the number of children, if any
...
const houseSize = (arr) => {
  return arr.map(character => {
    ...
    let members = 1;
    // Add one if the character has a spouse
    if (character.spouse) members += 1;
    // Add the number of children
    if (character.children) members += character.children.length;
    // Return an object representing the house and its size
    return { house: character.house, members };
  });
};
...
const houseSurvivors = (arr) => {
  return arr.map(character => {
    // Start with 1 for the character themselves
    let members = 1;
    // Add one if the character has a spouse not listed as deceased
    if (character.spouse && !deceasedSpouses.includes(character.spouse)) members += 1;
    // Add the number of children
    if (character.children) members += character.children.length;
    // Return an object representing the house and its surviving members
    return { house: character.house, members };
  });
};
